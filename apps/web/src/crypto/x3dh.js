...
function kdf(dh1, dh2, dh3, dh4) {
  const input = new Uint8Array(dh1.length + dh2.length + dh3.length + (dh4 ? dh4.length : 0));
  let offset = 0;
  input.set(dh1, offset); offset += dh1.length;
  input.set(dh2, offset); offset += dh2.length;
  input.set(dh3, offset); offset += dh3.length;
...
export function x3dhInitiator(myIdentity, myEphemeral, theirBundle) {
  if (!nacl.sign.detached.verify(theirBundle.signedPrekey, theirBundle.signedPrekeySignature, theirBundle.identityKey))
    throw new Error("invalid signed prekey signature");

  const identityDHSecret = nacl.box.keyPair.fromSecretKey(myIdentity.secretKey.slice(0, 32)).secretKey;
  ...
  const dh4 = theirBundle.usedOneTimePrekey ? dh(myEphemeral.secretKey, theirBundle.usedOneTimePrekey) : null;
...
export function x3dhResponder(myIdentity, mySignedPrekey, myOneTimePrekey, theirIdentityKey, theirEphemeralKey) {
  const identityDHSecret = nacl.box.keyPair.fromSecretKey(myIdentity.secretKey.slice(0, 32)).secretKey;
  ...
  const dh4 = myOneTimePrekey ? dh(myOneTimePrekey.secretKey, theirEphemeralKey) : null;
