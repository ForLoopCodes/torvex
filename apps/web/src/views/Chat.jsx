...
function saveRatchetState(myPk, peerPk, state) {
  try { sessionStorage.setItem(`${STORAGE_PREFIX}${myPk}:${peerPk}`, serializeState(state)); } catch {}
}

async function fetchPrekeyBundle(token, pubkey) {
  const res = await fetch(`${API}/keys/bundle/${pubkey}`, { headers: { Authorization: `Bearer ${token}` } });
  ...
  return {
    ...
    usedOneTimePrekey: d.oneTimePrekey ? bs58.decode(d.oneTimePrekey.publicKey) : null,
    usedOnePrekeyId: d.oneTimePrekey?.id ?? null,
  };
}

async function fetchPendingMessages(token) {
  const res = await fetch(`${API}/messages/pending`, { headers: { Authorization: `Bearer ${token}` } });
  if (!res.ok) return [];
...
export default function Chat({ session, onLogout }) {
  ...
  const [contacts, setContacts] = useState(() => {
    try { return JSON.parse(localStorage.getItem(`torvex_contacts_${session.pubkey}`) || "[]"); } catch { return []; }
  });
  ...
  useEffect(() => {
    localStorage.setItem(`torvex_contacts_${session.pubkey}`, JSON.stringify(contacts));
  }, [contacts, session.pubkey]);

  const getRatchet = useCallback((peerPk) => {
    ...
    if (loaded) { ratchetsRef.current.set(peerPk, loaded); return loaded; }
    return null;
  }, [session.pubkey]);

  const setRatchet = useCallback((peerPk, state) => {
    ratchetsRef.current.set(peerPk, state);
    saveRatchetState(session.pubkey, peerPk, state);
  }, [session.pubkey]);

  const resolveDisplayName = useCallback(async (pubkey) => {
  ...
  const peerLabel = useCallback((pk) => peerNames[pk] || shortKey(pk), [peerNames]);

  const initSessionWithPeer = useCallback(async (peerPk) => {
    if (!hasRatchetKeys || initializingRef.current.has(peerPk) || getRatchet(peerPk)) return;
    initializingRef.current.add(peerPk);
    try {
      const bundle = await fetchPrekeyBundle(session.token, peerPk);
      if (!bundle) {
        setMessages((prev) => [...prev, { id: Date.now().toString(), from: "system", text: `${peerLabel(peerPk)} has no prekey bundle`, ts: Date.now() }]);
        return;
      }
      ...
      wsRef.current?.send(JSON.stringify({
        type: "x3dh_init", to: peerPk,
        identityKey: bs58.encode(session.keys.identity.publicKey),
        ephemeralKey: bs58.encode(ephemeral.publicKey),
        usedOnePrekeyId,
        header: initMsg.header, nonce: initMsg.nonce, ciphertext: initMsg.ciphertext,
      }));
      setMessages((prev) => [...prev, { id: Date.now().toString(), from: "system", text: `ratchet session started with ${peerLabel(peerPk)}`, ts: Date.now() }]);
    } catch (err) {
  ...
  const handleMsg = useCallback((e) => {
    ...
    if (msg.type === "user_joined") {
      ...
      setTypingPeers((prev) => { const n = new Set(prev); n.delete(msg.pubkey); return n; });
      ...
      try {
        ...
        const myOtp = msg.usedOnePrekeyId != null ? session.oneTimePrekeys?.find((k) => k.id === msg.usedOnePrekeyId)?.keyPair : null;
        const sharedSecret = x3dhResponder(session.keys.identity, session.signedPrekey?.keyPair, myOtp || null, theirIdentity, theirEphemeral);
        ...
        setMessages((prev) => [...prev, { id: Date.now().toString(), from: "system", text: `ratchet session established with ${shortKey(msg.from)}`, ts: Date.now() }]);
      } catch (err) {
      ...
      setMessages((prev) => [...prev, { id: msg.id, from: msg.from, text, ts: msg.ts, read: false }]);
      if (wsRef.current?.readyState === 1) wsRef.current.send(JSON.stringify({ type: "read", to: msg.from, msgId: msg.id }));
    } else if (msg.type === "chat_ack") {
      const pending = pendingRef.current.get(msg.id);
      if (pending) {
        pendingRef.current.delete(msg.id);
        setMessages((prev) => [...prev, { id: msg.id, from: session.pubkey, text: pending.text, ts: msg.ts, peer: pending.peer, read: false }]);
      }
    } else if (msg.type === "typing") {
      if (msg.active) {
        setTypingPeers((prev) => new Set(prev).add(msg.from));
        setTimeout(() => setTypingPeers((prev) => { const n = new Set(prev); n.delete(msg.from); return n; }), TYPING_TIMEOUT + 500);
      } else {
        setTypingPeers((prev) => { const n = new Set(prev); n.delete(msg.from); return n; });
      }
  ...
  useEffect(() => {
    fetchDisplayName(session.token, session.pubkey).then((n) => { if (n) setMyName(n); });
  ...
  function sendTyping() {
    ...
    wsRef.current?.send(JSON.stringify({ type: "typing", to: activePeer, active: true }));
    clearTimeout(typingTimerRef.current);
    typingTimerRef.current = setTimeout(() => {
      wsRef.current?.send(JSON.stringify({ type: "typing", to: activePeer, active: false }));
    }, TYPING_TIMEOUT);
  }
  ...
  function send(e) {
    ...
    if (!state) {
      setMessages((prev) => [...prev, { id: Date.now().toString(), from: "system", text: `no session with ${peerLabel(activePeer)} — click their name to init`, ts: Date.now() }]);
      setInput("");
      return;
    }
    ...
    wsRef.current.send(JSON.stringify({ type: "chat", recipients: [{ to: activePeer, header, nonce, ciphertext }] }));
    setTimeout(() => {
      if (pendingRef.current.has(tempId)) {
        const p = pendingRef.current.get(tempId);
        pendingRef.current.delete(tempId);
        setMessages((prev) => [...prev, { id: tempId, from: session.pubkey, text: p.text, ts: Date.now(), peer: p.peer }]);
      }
    }, 3000);
    setInput("");
    wsRef.current.send(JSON.stringify({ type: "typing", to: activePeer, active: false }));
  ...
  async function handleSetName() {
    ...
    if (result) { setMyName(result); setShowNameEdit(false); setNameInput(""); }
  }
  ...
  const peerMsgs = activePeer ? messages.filter((m) => m.from === activePeer || (m.from === session.pubkey && m.peer === activePeer) || m.from === "system") : [];
  ...
  return (
    <div className="chat-layout">
      <aside className="sidebar">
        <div className="sidebar-header">
          ...
          <div className="user-info">
            <span className="user-badge" title={session.pubkey} onClick={() => setShowNameEdit(!showNameEdit)}>
              {myName || shortKey(session.pubkey)}
            </span>
            {showNameEdit && (
              <div className="name-edit">
                <input value={nameInput} onChange={(e) => setNameInput(e.target.value)} placeholder="display name" maxLength={32} />
        ...
        <div className="add-contact">
          <input value={addPeerInput} onChange={(e) => setAddPeerInput(e.target.value)} placeholder="add by pubkey..." />
        ...
        <div className="online-list">
          ...
          {allPeers.map((u) => (
            <div
              ...
              title={u}
              onClick={() => { setActivePeer(u); if (hasRatchetKeys && !getRatchet(u)) initSessionWithPeer(u); resolveDisplayName(u); }}
            >
      ...
      <main className="chat-main">
        {!activePeer ? (
          <div className="empty-chat"><p>select a peer to start chatting</p></div>
        ) : (
          <>
            <div className="chat-header">
              <h3>{peerLabel(activePeer)}</h3>
              <span className={`header-status ${onlineUsers.includes(activePeer) ? "online" : "offline"}`}>
                {onlineUsers.includes(activePeer) ? "online" : "offline"}
              </span>
              {typingPeers.has(activePeer) && <span className="typing-indicator">typing...</span>}
            </div>
            <div className="messages">
              {peerMsgs.length === 0 && <div className="empty-chat"><p>no messages yet. say something.</p></div>}
              {peerMsgs.map((m) => (
                <div key={m.id} className={`msg ${m.from === session.pubkey ? "msg-self" : m.from === "system" ? "msg-system" : "msg-other"}`}>
                  {m.from !== "system" && <span className="msg-author">{m.from === session.pubkey ? (myName || "you") : peerLabel(m.from)}</span>}
                  <span className="msg-text">{m.text}</span>
                  <span className="msg-meta">
                    <span className="msg-time">{new Date(m.ts).toLocaleTimeString()}</span>
                    {m.from === session.pubkey && <span className="msg-check">{readReceipts[m.id] ? "✓✓" : "✓"}</span>}
                  </span>
                </div>
              ))}
              <div ref={bottomRef} />
